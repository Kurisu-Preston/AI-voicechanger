...
from infer_tools.infer_tool_beta import Svcb
...
def voice_change_model():
    ...
    f_pitch_change, input_wav_path, batch = svc_model.infer_autokey(input_wav_path, key_adjust=f_pitch_change, acc=accelerate, use_crepe=False)

    _f0_tst, _f0_pred, _audio = svc_model.infer(input_wav_path, key=f_pitch_change, batch=batch)
...
if __name__ == '__main__':
    ...
    svc_model = Svcb(project_name, config_path, hubert_gpu, model_path)

    # 此处与vst插件对应，不建议更改
    app.run(port=6847, host="0.0.0.0", debug=False, threaded=False)
